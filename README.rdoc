...
==PT/BR
<b>Para configurar o sistema localmente</b>
  echo 'rvm --create 1.9.2@mini_shop' >> .rvmrc
  gem install bundler
  bundler install

<b>Configurar a base</b>
 ...
 - configurar usuários e bases do postgresql
   ...
   <tt>$ cp config/database.sample.yml config/database.yml</tt>

<b>**opcional**</b> caso use o postgresql-8.4, deve ser alterado a conf de metodo de autenticação local.

   <tt>$ sudo vim /etc/postgresql/8.4/main/pg_hba.conf</tt>

   local   all         postgres                     trust
   local   all         all                          trust

   <tt>$ sudo /etc/init.d/postgresql-8.4 restart</tt>